...
/**
  * CHANGEZ l'ID du drone par celui que Alphatango vous a fourni (Trigramme + ModÃ¨le + numÃ©ro sÃ©rie) !
  */
...
// Le wifi est sur le channel 6 conformement Ã  la spÃ©cification
...
void setup()
{
    ...
    delay(1000);
    //initialisation du BN220. Bien penser Ã  croiser Rx et Tx entre ESP32 et le BN220!
 ...
 */
    /**
     * Pour mon exemple, je crÃ©e un point d'accÃ©s. Il fait rien par defaut.
     */
...
/**
 * DÃ©but du code principal. C'est une boucle infinie.
 */
void loop()
{
    ...
    switch (program) {
    case 0:
        // Ici on lit les donnÃ©es qui arrive du GPS et on les passes Ã  la librairie TinyGPS++ pour les traiter
        ...
        if (!gps.location.isValid()) {
            ...
            // On renseigne le point de dÃ©marrage quand la prÃ©cision est satisfaisante
            if (!has_set_home && gps.satellites.value() > 6 && gps.hdop.hdop() < 2.0) {
                ...
                Serial.println("Altitude de dÃ©part="+String(home_alt));
            ...
            // On envoie les donnÃ©es a la librairie d'identification drone pour le formattage.
            ...
            // Ici on ecrit sur le port USB les donnÃ©es GPS pour visualisation seulement.
            if (millis() - gpsMap > 1000) {
    ...
    */
     ...
     *  - soit toutes les 3s,
     *  - soit si le drone s'est dÃ©placÃ© de 30m en moins de 30s soit 10m/s ou 36km/h,
     *  - uniquement si la position Home est dÃ©jÃ  dÃ©finie,
     *  - et dans le cas oÃ¹ les donnÃ©es GPS sont nouvelles.
     */
    if (drone_idfr.has_home_set() && drone_idfr.time_to_send()) {
        ...
        /**
         * On gÃ©nÃ¨re la trame wifi avec l'identfication
         */
        const uint8_t to_send = drone_idfr.generate_beacon_frame(beaconPacket, header_size);  // override the null termination
        // DÃ©commenter ce block pour voir la trame entiÃ¨re sur le port usb
